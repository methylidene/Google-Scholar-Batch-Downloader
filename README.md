...
一个本地运行的 Chrome Manifest V3 扩展，用于在**当前 Google Scholar 搜索结果页或作者主页**选择论文、批量下载可用的公开 PDF，汇报每篇论文的最终下载状态，并导出 RIS、BibTeX、JSON 和 CSV。
...
批次结束后，Scholar 页面右侧会显示总数、成功下载、未找到 PDF、下载失败、下载超时和导出错误数量。展开“逐篇明细”可查看标题、最终状态、文件名、来源、PDF URL 和失败原因；点击“关闭汇报”可移除面板，开始新批次时旧汇报也会自动移除。
...
自动下载的 CSV 使用 UTF-8 BOM，可在 Windows Excel 中正确显示中文。字段依次为 `title`、`authors`、`year`、`status`、`source`、`pdfUrl`、`filename`、`downloadId`、`error`、`startedAt` 和 `finishedAt`。刷新或关闭 Scholar 页面后不会恢复未完成的页面汇报。

当前版本的 `source` 固定为 `scholar`。备用下载源和自定义站内检索将在后续版本实现；本版本下载失败后不会自动访问其他论文网站。
